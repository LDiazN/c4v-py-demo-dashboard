...
# Limit
use_crawl_limit = sl.sidebar.checkbox("Limit", help="Use a max amount of urls to be crawled at the same time", value=True)
if use_crawl_limit:
    crawl_limit = sl.sidebar.number_input("Max instances to add", 0, value=100)
else:
    crawl_limit = -1
# Crawlers to use
crawlers = sl.sidebar.multiselect("Crawl for: ", options=app.manager.get_available_crawlers(), help="Sites that can be scraped for new data")
...
def run_crawl_callback():
    ...
    if not crawlers or crawl_limit == 0: return

    # Start classification process
    sl.info("Starting crawling process")
    app.crawl(crawlers, crawl_limit, generate_progress_finite() if crawl_limit > 0 else generate_progress_infinite())
